...
#### 서버 관리
##### Linux (systemd)
- **시작:** `sudo systemctl start cfrp-server`
- **중지:** `sudo systemctl stop cfrp-server`
- **상태:** `systemctl status cfrp-server`
- **로그:** `journalctl -u cfrp-server -f`

##### macOS (launchd)
- **시작:** `sudo launchctl load -w /Library/LaunchDaemons/com.inarms.cfrp-server.plist`
- **중지:** `sudo launchctl unload /Library/LaunchDaemons/com.inarms.cfrp-server.plist`
- **상태:** `sudo launchctl list | grep cfrp-server`
- **로그:** `tail -f /var/log/cfrp-server.log`

##### Windows (PowerShell)
- **시작:** `Start-Service cfrp-server`
- **중지:** `Stop-Service cfrp-server`
- **상태:** `Get-Service cfrp-server`
- **로그:** `C:\Program Files\cfrp\cfrp.log` 확인
...
#### 클라이언트 관리
##### Linux (systemd)
- **시작:** `sudo systemctl start cfrp-client`
- **중지:** `sudo systemctl stop cfrp-client`
- **재시작:** `sudo systemctl restart cfrp-client`

##### macOS (launchd)
- **시작:** `sudo launchctl load -w /Library/LaunchDaemons/com.inarms.cfrp-client.plist`
- **중지:** `sudo launchctl unload /Library/LaunchDaemons/com.inarms.cfrp-client.plist`
- **재시작:** `sudo launchctl unload /Library/LaunchDaemons/com.inarms.cfrp-client.plist && sudo launchctl load -w /Library/LaunchDaemons/com.inarms.cfrp-client.plist`

##### Windows (PowerShell)
- **시작:** `Start-Service cfrp-client`
- **중지:** `Stop-Service cfrp-client`
- **재시작:** `Restart-Service cfrp-client`
