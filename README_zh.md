...
- **轻量级**: 极简依赖 (`asio`, `tomlplusplus`, `wolfssl`, `ngtcp2`)。使用紧凑的 **自定义二进制协议** 以实现极低的开销。
...
2. **控制流**: 用于交换指令的虚拟流，使用 **自定义二进制序列化** 协议.
...
### 配置文件自动选择
**注意：如果当前目录下存在 `server.toml` 或 `client.toml`，它们将具有绝对优先级。所有命令行位置参数和选项（如 `-c` 或 `-t`）都将被忽略。**

如果不存在配置文件，程序将遵循以下逻辑：
1. **位置参数**: 如果提供了路径（例如 `./cfrp my.toml`），则使用该文件。
2. **选项**: 如果同时提供了 `-c` 和 `-t`，则使用这些值生成 `client.toml`。
3. **默认**: 否则，生成默认的 `server.toml` 并以服务端模式启动。
...
#### 3. 快速客户端设置 (自动生成)
如果你有服务端的 CA 证书 (`ca.crt`) 和令牌 (Token)，可以快速启动客户端。如果配置文件不存在，`cfrp` 会自动生成 `client.toml`:
```bash
./cfrp -c certs/ca.crt -t your_secret_token
```
- **强制进入客户端模式**，即使存在 `server.toml`。
- 如果 `client.toml` **不存在**，将自动生成一个并配置为使用提供的 CA 进行验证和提供的令牌。
- 如果 `client.toml` **已存在**，将直接使用现有配置（忽略 `-c` 和 `-t` 参数）。
